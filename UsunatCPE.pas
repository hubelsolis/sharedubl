...
uses
  ...
  Vcl.Buttons, Xml.XMLDoc, AbBase, AbBrowse, AbZBrows, AbZipper,
  Soap.InvokeRegistry, IdBaseComponent, IdCoder, IdCoder3to4, IdCoderMIME,
  Soap.Rio, Soap.SOAPHTTPClient,Vcl.Clipbrd, System.Net.URLClient;

const
  CEROS_PRECIOS = 6;
  CEROS_XML = 2;
  CEROS_XML_CANTIDAD = 6;
...
type
  ...
  public
    ...
    function GeneraCPESN(serie,numero:string):string;
...
implementation
Uses UBLInvoice21,UBLCreditNote21,UBLDebitNote21,UBLPESummaryDocuments10,UBLDespatchAdvice21,
     //billServiceBETA,//ACBrDFeSSL,ACBrUtil,
     Operadores,types,Math,USQLUtils,UDBModulo, Ureport,USunatVariables,
     Uconfiguracion;

{$R *.dfm}

{$DEFINE USAR_NETFRAMEWORK}
 ...
 function ACBrfirmaXML(xmlsinfirmar,xmlfirmado,pathcertificado,clavecertificado:WideString; conformato : integer):Integer;
  var xmltmp  : TXMLDocument;
   ...
   begin
      ...
      xmltmp.Active:=True;
      xmltmp.Encoding :=codificadoXML;  // utf-8   or  'ISO-8859-1'
      ...
      if (conformato=1) and (false) then
...
begin
  ...
  try
    if FileExists(workdir+'\facturador.ini') then
    begin
      Lista.LoadFromFile(workdir+'\facturador.ini');
      for i := 0 to Lista.Count - 1 do
      begin
        clave := SplitCadena(Lista[i], '=', 1);
        if clave = nombre then
        begin
          valor := SplitCadena(Lista[i], '=', 2);
          Result := valor;
          Exit;
        end;
      end;
...
procedure activaDM1ZregVen(d:string);
 begin
    DM1.Zregven.SQL.clear;
    DM1.ZRegVen.SQL.Add(FormatSQL3(
    //        0            1        2          3               4          5          6              7                8
    'select a.DUAL,a.FECHAEMISION,a.HORA,a.TIPOCOMPROBANTE,a.SERIECO,a.NUMEROCO,a.TIPODOCUMENTO,a.NRODOCUMENTO,a.RAZONNOMBRE, '+
    //  9              10                                11                              12
    ...
    //       17                           18                      19                                 20                21        22
    'abs(a.DESCUENTOS) AS DESCUENTOS,abs(a.CARGOS) AS CARGOS, abs(a.IMPORTETOTAL) AS IMPORTETOTAL,a.GUIAREMISION,a.TIPOGUIA, a.PLACA, '+
    //      23                           24       25           26         27            28          29               30
    'abs(a.PERCEPCION) AS PERCEPCION,a.RCPTIPO,a.RCPSERIE,a.RCPNUMERO,a.CODIGONOTA,a.MOTIVONOTA,a.TIPOPAGO,a.FECHAVENCIMIENTO, '+
    //    31          32           33       34                   35                                    36
    'a.TIPODSCTO, a.DSCTOUNIT, a.IGVTASA, a.ENVIOSUNAT, abs(a.DSCTOGLOBAL) as DSCTOGLOBAL , abs(a.CARGOGLOBAL) as CARGOGLOBAL, '+
    //     37           38             39            40         41           42             43
    'a.CAT15_ELEM1,a.CAT15_ELEM2,a.CAT15_ELEM3,a.CAT15_ELEM4,a.GLOSA,a.TIPOOPERACION,a.TOTALANTICIPOS '+
...
begin
  ...
  DM1.ZGuiaRem.SQL.Add(FormatSQL3(
   ...
   //      23                 24                 25                26            27           28               29          30
   'a.TRASBORDOPROGRAMADO, a.UNIDADPESO, a.TOTALPAQUETES, a.TRANSPORTMODECODE, a.DNIS, a.UBIGEOORIGEN, a.UBIGEODESTINO, a.SCOP '+
   'FROM GUIAREMISION a '+
...
begin
 ...
 if True then //chkXML.Checked
   begin
     ...
     //if FrmServidorCPE.chksinenvio.Checked then
       pe:='and enviosunat=0';
     //else pe:='';
...
var error : boolean;
    scertificado, Spasscertificado : string;
begin
  ...
  if passcertificado<>'' then Spasscertificado:=passcertificado
  ELSE
    spasscertificado:=get_variable_sfs('CLAVE_CERTIFICADO');
...
var patharchivo,archivo,TMP : string;
 ...
 begin
  ...
  if ValI(dual)>0 then
   BEGIN                    //      0               1             2          3         4
    ActivaDM1UtilSQL('Select r.FECHAEMISION,r.TIPOCOMPROBANTE,r.SERIECO,r.NUMEROCO,r.TIPOGUIA '+
    ...
    if (DM1.UtilSQL.RecordCount>0) then
      begin
        ...
        //01/02/2018
        //1234567890
        patharchivo:=WorkDir+'\CPE\'+Copy(DM1.UtilSQL.Fields[0].AsString,7,4)+'\'+Copy(DM1.UtilSQL.Fields[0].AsString,4,2)+'\DATA\';
        ...
        archivo:=DM1.UtilSQL.Fields[1].AsString; // tipodocumento
        if DM1.UtilSQL.Fields[1].AsString='10' then
          archivo:=DM1.UtilSQL.Fields[4].AsString;  // pero si es guia remision segun el tipo de guia

        archivo:=
        RUC_EMPRESA+'-'+
        CompletarCadena( archivo ,'0',2)+'-'+
        DM1.UtilSQL.Fields[2].AsString+'-'+
        CompletarCadena( DM1.UtilSQL.Fields[3].AsString, '0',8)+'.xml';
...
//          'RC'+'-'+copy(fecharesumen,1,8)+'-'+completarCadena(StrI(NRO_RESUMEN_INICIAL+i),'0',3)+'.xml';
          'RC'+'-'+copy(fecharesumen,1,8)+'-'+StrI(ValI(get_variable_sfs('CODIGO_LOCAL')))+
...
function TFrmGenCPE.GeneraCPESN(serie,numero:string):string;
var dd : string;
 begin
   ActivaDM1ConsultaSQL('select dual from registroventas where serieco="'+serie+'" and numeroco="'+numero+'"',false);
   dd:=DM1.ConsultaSQL.Fields[0].AsString;
   DM1.ConsultaSQL.Close;
   if dd<>'' then
     result:=FrmGenCPE.GeneraCPE(dd,'' ,false)
   else
     result:='NAND';
 end;
...
var cpe : IXMLInvoiceType;
 ...
 begin
  ...
  //2024 jefe puso este tipo de documento
  if (NRO_DOC_CLIENTE='99999999') or (NRO_DOC_CLIENTE='00000000') or (NRO_DOC_CLIENTE='12345678') then
   begin
       TIPO_DOC_CLIENTE:='0';
   end;
  ...
  if (TIPO_DOC_CLIENTE='0') THEN     // BOLETA SIN DOCUMENTO
   ...
   END;
    ...
    if TIPO_DOC_CLIENTE='4' then
     IF LENGTH(NRO_DOC_CLIENTE)<9 THEN  //if NOT (CARNET_EXTRANJERIA='SI') then
       begin
          TIPO_DOC_CLIENTE:='0';
          NRO_DOC_CLIENTE:='-';
        end;
  ...
  //2025
  conDetraccion:=False;
   ActivaDM1ConsultaSQL('select dual from detraccion where dual='+DM1.ZRegVen.Fields[0].AsString,true);
   if DM1.ConsultaSQL.RecordCount>0 then conDetraccion:=True;
  DM1.ConsultaSQL.Close;
  ...
  //2025
  if conDetraccion then
    cpe.InvoiceTypeCode.SetAttributeNS('listID','','1004')                                    // factura con detraccion
  else
    cpe.InvoiceTypeCode.SetAttributeNS('listID','','01'+'01');     // venta interna0101, 0104 anticipo  DM1.ZRegVen.fields[42].AsString
  ...
  if ENVIAR_GLOSA_A_SUNAT='SI' then
   begin
     if trim(DM1.ZRegVen.Fields[41].AsString)<>'' then
      begin
       cpe.Note.Add;
       cpe.Note.Items[cpe.Note.Count-1].Text:=Trim(DM1.ZRegVen.Fields[41].AsString);
       //cpe.Note.Items[cpe.Note.Count-1].Text:='<![CDATA['+Trim(DM1.ZRegVen.Fields[41].AsString)+']]>';
      end;
   end;
  ...
  cpe.Signature.Add;
   ...
   cpe.AccountingSupplierParty.Party.PartyLegalEntity.Add;
     ...
     cpe.AccountingSupplierParty.Party.PartyLegalEntity.Items[0].RegistrationAddress.AddressTypeCode.Text:=get_variable_sfs('CODIGO_LOCAL'); //FrmServidorCPE.edtCodLocal.text;  // CODIGO_LOCAL_ANEXO
  ...
  if conDetraccion then
   begin
    //pediente
    {
        ...
        <cac:Party>
          <cac:PostalAddress>
                <cbc:ID>221005</cbc:ID>
                <cbc:CitySubdivisionName/>
                <cbc:CityName>TOCACHE</cbc:CityName>
                <cbc:CountrySubentity>SAN MARTIN</cbc:CountrySubentity>
                <cbc:District>UCHIZA </cbc:District>
              <cac:AddressLine>
                <cbc:Line><![CDATA[---- SAN JUAN DE PORONGO S/N ---- SAN JUAN DE PORONGO SUMINISTRO 126379 SAN MARTIN-TOCACHE-UCHIZA]]></cbc:Line>
              </cac:AddressLine>
    ...
    ActivaDM1ConsultaSQL('select d.MEDIOPAGO,d.CUENTAPAGO,d.TIPODETRACCION,d.PORCENTAJEDETRACCION,d.TOTALDETRACCION from detraccion d where d.dual='+DM1.ZRegVen.Fields[0].AsString,true);
    cpe.PaymentMeans.Add;
      cpe.PaymentMeans.Items[0].ID.Text:='Detraccion';

      cpe.PaymentMeans.Items[0].PaymentMeansCode.SetAttributeNS('listName','','Medio de pago');
      cpe.PaymentMeans.Items[0].PaymentMeansCode.SetAttributeNS('listAgencyName','','PE:SUNAT');
      cpe.PaymentMeans.Items[0].PaymentMeansCode.SetAttributeNS('listURI','','urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo59');
      cpe.PaymentMeans.Items[0].PaymentMeansCode.text:=DM1.ConsultaSQL.Fields[0].AsString;

      cpe.PaymentMeans.Items[0].PayeeFinancialAccount.ID.Text:=DM1.ConsultaSQL.Fields[1].AsString;

    cpe.PaymentTerms.Add; inc(ipytr);
      cpe.PaymentTerms.Items[ipytr].ID.Text:='Detraccion';

      cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Add;
        cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Items[0].SetAttributeNS('schemeName','','Codigo de detraccion');
        cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Items[0].SetAttributeNS('schemeAgencyName','','PE:SUNAT');
        cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Items[0].SetAttributeNS('schemeURI','','urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo54');
        cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Items[0].text:=DM1.ConsultaSQL.Fields[2].AsString;;

      cpe.PaymentTerms.Items[ipytr].PaymentPercent.Text:=DM1.ConsultaSQL.Fields[3].AsString;
      cpe.PaymentTerms.Items[ipytr].Amount.SetAttributeNS('currencyID','',MONEDA);
      cpe.PaymentTerms.Items[ipytr].Amount.Text:=DM1.ConsultaSQL.Fields[4].AsString;
  ...
  // 03/03/2021
  // 06/02/2025 se agrego cronograma de pagos
  // CREDITO
  if (DM1.ZRegVen.Fields[29].AsString='1') and ((Variables.get('FORZAR_CONTADO','')<>'SI')) then
   begin

     cpe.PaymentTerms.Add; inc(ipytr);
       cpe.PaymentTerms.Items[ipytr].ID.Text:='FormaPago';
       cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Add;
       cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Items[0].Text:='Credito';
       cpe.PaymentTerms.Items[ipytr].Amount.Text:=IMPORTE_TOTAL;
       cpe.PaymentTerms.Items[ipytr].Amount.SetAttributeNS('currencyID','',MONEDA);

     ActivaDM1ConsultaSQL('Select r.FECHA, r.MONTO from cronogramacreditos r where dual='+DM1.ZRegVen.Fields[0].AsString+' order by nro',true);
     if DM1.ConsultaSQL.RecordCount>0 then
      begin
       for i := 1 to DM1.ConsultaSQL.RecordCount do
        begin
           if i>1 Then DM1.ConsultaSQL.Next;

           cpe.PaymentTerms.Add; inc(ipytr);
            cpe.PaymentTerms.Items[ipytr].ID.text:='FormaPago';

           cpe.PaymentTerms.Items[ipytr].PaymentMeansID.add;
             cpe.PaymentTerms.Items[ipytr].PaymentMeansID.items[0].Text:='Cuota'+CompletarCadena(StrI(i),'0',3);
           cpe.PaymentTerms.Items[ipytr].Amount.Text:=DM1.ConsultaSQL.Fields[1].AsString;
           cpe.PaymentTerms.Items[ipytr].Amount.SetAttributeNS('currencyID','',MONEDA);
           if BuscaSTR(DM1.ConsultaSQL.Fields[0].AsString,'-') then
              cpe.PaymentTerms.Items[ipytr].PaymentDueDate.Text:=DM1.ConsultaSQL.Fields[0].AsString
           else
              cpe.PaymentTerms.Items[ipytr].PaymentDueDate.Text:=Copy(DM1.ConsultaSQL.Fields[0].AsString,7,4)+'-'+
                                Copy(DM1.ConsultaSQL.Fields[0].AsString,4,2)+'-'+  //
                                Copy(DM1.ConsultaSQL.Fields[0].AsString,1,2);      // fecha de vencimiento
        end;
      end
     else
      begin // No hay cronograma quiere decir a una sola fecha

       cpe.PaymentTerms.Add; inc(ipytr);
         cpe.PaymentTerms.Items[ipytr].ID.Text:='FormaPago';
         cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Add;
         cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Items[0].Text:='Cuota001';
         cpe.PaymentTerms.Items[ipytr].Amount.Text:=IMPORTE_TOTAL;
         cpe.PaymentTerms.Items[ipytr].Amount.SetAttributeNS('currencyID','',MONEDA);
         if BuscaSTR(DM1.ZRegVen.Fields[30].AsString,'-') then
            cpe.PaymentTerms.Items[ipytr].PaymentDueDate.Text:=DM1.ZRegVen.Fields[30].AsString
         else
            cpe.PaymentTerms.Items[ipytr].PaymentDueDate.Text:=Copy(DM1.ZRegVen.Fields[30].AsString,7,4)+'-'+
   ...
   begin
     cpe.PaymentTerms.Add; inc(ipytr);
       cpe.PaymentTerms.Items[ipytr].ID.Text:='FormaPago';
       cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Add;
       cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Items[0].Text:='Contado';
       cpe.PaymentTerms.Items[ipytr].Amount.Text:=IMPORTE_TOTAL;
       cpe.PaymentTerms.Items[ipytr].Amount.SetAttributeNS('currencyID','',MONEDA);
   end;
    ...
    for i := 0 to DM1.ZDetVen.RecordCount-1 do
     begin
      ...
      cpe.InvoiceLine.Add;
        ...
        //13/01/2025 detracciones requieren ruta
        if condetraccion then
         begin
           ActivaDM1ConsultaSQL('select d.DETALLEVIAJE,d.UBIGEOORIGEN,d.DIRECCIONORIGEN,d.VRSERVICIOTRANSPORTE,d.VRCARGAEFECTIVA,d.VRCARGAUTIL,d.UBIGEODESTINO,d.DIRECCIONDESTINO from DETRACCION d where d.dual='+DM1.ZRegVen.Fields[0].AsString,true);
           ipytr:=-1;
           cpe.InvoiceLine.Items[i].Delivery.Add; inc(ipytr);
             cpe.InvoiceLine.Items[i].Delivery.items[ipytr].Despatch.Instructions.Add;
                cpe.InvoiceLine.Items[i].Delivery.items[ipytr].Despatch.Instructions.items[0].Text:=DM1.ConsultaSQL.Fields[0].AsString;
             cpe.InvoiceLine.Items[i].Delivery.items[ipytr].Despatch.DespatchAddress.ID.SetAttributeNS('schemeAgencyName','','PE:INEI');
             cpe.InvoiceLine.Items[i].Delivery.items[ipytr].Despatch.DespatchAddress.ID.SetAttributeNS('schemeName','','Ubigeos');
             cpe.InvoiceLine.Items[i].Delivery.items[ipytr].Despatch.DespatchAddress.ID.Text:=DM1.ConsultaSQL.Fields[1].AsString;
             cpe.InvoiceLine.Items[i].Delivery.items[ipytr].Despatch.DespatchAddress.AddressLine.Add;
                cpe.InvoiceLine.Items[i].Delivery.items[ipytr].Despatch.DespatchAddress.AddressLine.Items[0].Line.text:=DM1.ConsultaSQL.Fields[2].AsString;

           if DM1.ConsultaSQL.Fields[3].AsString<>'' then
            begin
             cpe.InvoiceLine.Items[i].Delivery.Add; inc(ipytr);
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.Add;
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.items[0].ID.text:='01';
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.items[0].Amount.SetAttributeNS('currencyID','',MONEDA);
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.items[0].Amount.text:=StrR(ValR(DM1.ConsultaSQL.Fields[3].AsString));
            end;

           if DM1.ConsultaSQL.Fields[4].AsString<>'' then
            begin
             cpe.InvoiceLine.Items[i].Delivery.Add; inc(ipytr);
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.Add;
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.items[0].ID.text:='02';
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.items[0].Amount.SetAttributeNS('currencyID','',MONEDA);
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.items[0].Amount.text:=StrR(ValR(DM1.ConsultaSQL.Fields[4].AsString));
            end;

           if DM1.ConsultaSQL.Fields[5].AsString<>'' then
            begin
             cpe.InvoiceLine.Items[i].Delivery.Add; inc(ipytr);
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.Add;
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.items[0].ID.text:='03';
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.items[0].Amount.SetAttributeNS('currencyID','',MONEDA);
               cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryTerms.items[0].Amount.text:=StrR(ValR(DM1.ConsultaSQL.Fields[5].AsString));
            end;

           cpe.InvoiceLine.Items[i].Delivery.Add; inc(ipytr);
             cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryLocation.Address.ID.SetAttributeNS('schemeAgencyName','','PE:INEI');
             cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryLocation.Address.ID.SetAttributeNS('schemeName','','Ubigeos');
             cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryLocation.Address.ID.Text:=DM1.ConsultaSQL.Fields[6].AsString;
             cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryLocation.Address.AddressLine.Add;
                cpe.InvoiceLine.Items[i].Delivery.items[ipytr].DeliveryLocation.Address.AddressLine.Items[0].Line.text:=DM1.ConsultaSQL.Fields[7].AsString;
           DM1.ConsultaSQL.Close;
         end;
...
var cpe : IXMLCreditNoteType;
    ...
    TOTAL_IGV, TOTAL_IMPUESTOS, TOTAL_DESCUENTOS, TOTAL_CARGOS, IMPORTE_TOTAL,
    TOTAL_BRUTO,TIPO_DOC_CLIENTE, NRO_DOC_CLIENTE,RAZON_NOMBRE_CLIENTE : string;
    DSCTO_GLOBAL, CARGO_GLOBAL,OTROS_TRIBUTOS,IMPORTE_TOTAL_RCP : string;
 ...
 begin
  ...
  cpe.Signature.Add;
   ...
   //2025 en caso de la nota de credito a facturas/boletas de credito se pueden modificar las fechas de pago
  //     22                 23                   24         25          26          27          28           29             30
  //' a.PLACA,abs(a.PERCEPCION) AS PERCEPCION,a.RCPTIPO,a.RCPSERIE,a.RCPNUMERO,a.CODIGONOTA,a.MOTIVONOTA,a.TIPOPAGO,a.FECHAVENCIMIENTO '+
  ipytr:=-1;
  if (DM1.ZRegVen.Fields[27].AsString='13') then  // nota de credito por cambio de fechas
     begin

       ActivaDM1ConsultaSQL('Select r.ImporteTotal from REGISTROVENTAS r where serieCO="'+DM1.ZRegVen.Fields[25].AsString+'" and numeroCO='+DM1.ZRegVen.Fields[26].AsString,true);
       IMPORTE_TOTAL_RCP:=StrR2(ValR(DM1.ConsultaSQL.Fields[0].AsString),12,CEROS_XML);
       cpe.PaymentTerms.Add; inc(ipytr);
         cpe.PaymentTerms.Items[ipytr].ID.Text:='FormaPago';
         cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Add;
         cpe.PaymentTerms.Items[ipytr].PaymentMeansID.Items[0].Text:='Credito';
         cpe.PaymentTerms.Items[ipytr].Amount.Text:=IMPORTE_TOTAL_RCP;
         cpe.PaymentTerms.Items[ipytr].Amount.SetAttributeNS('currencyID','',MONEDA);
    ...
    for i := 0 to DM1.ZDetVen.RecordCount-1 do
     begin
      ...
      cpe.CreditNoteLine.Add;
        ...
        // INFORMACION DE DESCUENTOS
        INFORMA_DESCUENTO:=FALSE;
        if (ValI(DM1.ZDetVen.Fields[7].AsString)<>0)  then  //amarilis  2022
          begin
            if FALSE THEN
             BEGIN
        ...
        IF (DM1.ZDetVen.Fields[12].AsString='1004') then
          ...
          begin
            if NOT INFORMA_DESCUENTO then
              cpe.CreditNoteLine.Items[i].Price.PriceAmount.Text:=StrR2( ValR(DM1.ZDetVen.Fields[6].AsString),12,CEROS_PRECIOS)   // desagregado
            else
              cpe.CreditNoteLine.Items[i].Price.PriceAmount.Text:=
                StrR2( ValR(DM1.ZDetVen.Fields[9].AsString)/ValR(DM1.ZDetVen.Fields[1].AsString) ,12,CEROS_PRECIOS);  // por dscto
          end;
...
var cpe : IXMLDebitNoteType;
 ...
 begin
  ...
  ' a.PLACA,abs(a.PERCEPCION) AS PERCEPCION,a.RCPTIPO,a.RCPSERIE,a.RCPNUMERO,a.CODIGONOTA,a.MOTIVONOTA,a.TIPOPAGO,a.FECHAVENCIMIENTO '+
        31         32        33         34
    TIPODSCTO, DSCTOUNIT, IGVTASA, TIPOOPERACION
    ...
    for i := 0 to DM1.ZDetVen.RecordCount-1 do
     begin
      ...
      cpe.DebitNoteLine.Add;
        ...
        // INFORMACION DE DESCUENTOS
        if (ValI(DM1.ZDetVen.Fields[7].AsString)<>0) and (true) then  //amarilis
          begin
...
var cpe : IXMLDespatchAdviceType;
    ...
    TIPO_DOC_CLIENTE, NRO_DOC_CLIENTE,RAZON_NOMBRE_CLIENTE,VPLACA : string;
 ...
 begin
  ...
  cpe.UBLVersionID.Text:='2.1';
  cpe.CustomizationID.Text:='2.0';
  //2022cpe.CustomizationID.SetAttributeNS('schemeAgencyName','','PE:SUNAT');
  ...
  cpe.DespatchAdviceTypeCode.SetAttributeNS('listAgencyName','','PE:SUNAT');
  cpe.DespatchAdviceTypeCode.SetAttributeNS('listName','','Tipo de Documento');
  cpe.DespatchAdviceTypeCode.SetAttributeNS('listURI','','urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo01');
  //cpe.Note.Add;
  ...
  cpe.Signature.Add;                                                                             // Firma del Sw y Firma Empresa
   ...
   //INFORMACION DEL REMITENTE DespatchSupplierParty 2022
   ...
   cpe.DespatchSupplierParty.Party.PartyIdentification.Add;
       cpe.DespatchSupplierParty.Party.PartyIdentification.Items[0].ID.Text:=RUC_EMPRESA;
       cpe.DespatchSupplierParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeID','','6');
       cpe.DespatchSupplierParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeName','','Documento de Identidad');
       cpe.DespatchSupplierParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeAgencyName','','PE:SUNAT');
       cpe.DespatchSupplierParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeURI','','urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06');

   cpe.DespatchSupplierParty.Party.PartyName.Add;
       cpe.DespatchSupplierParty.Party.PartyName.Items[0].Name.text:=EliminaProhibidos(RAZON_EMPRESA);
   ...
   {
     cpe.DespatchSupplierParty.Party.PartyLegalEntity.Items[0].PartyIdentification.add;
     if not concdata then
       cpe.DespatchSupplierParty.Party.PartyLegalEntity.Items[0].RegistrationName.Text:=EliminaProhibidos(RAZON_EMPRESA)
     else
   ...
   //INFORMACION DEL LA ENTREGA AL CLIENTE DESTINATARIO DeliveryCustomerParty
   cpe.DeliveryCustomerParty.Party.PartyIdentification.Add;
       cpe.DeliveryCustomerParty.Party.PartyIdentification.Items[0].ID.Text:=NRO_DOC_CLIENTE;
       cpe.DeliveryCustomerParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeID','',TIPO_DOC_CLIENTE);
       cpe.DeliveryCustomerParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeName','','Documento de Identidad');
       cpe.DeliveryCustomerParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeAgencyName','','PE:SUNAT');
       cpe.DeliveryCustomerParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeURI','','urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06');
   ...
   cpe.DeliveryCustomerParty.CustomerAssignedAccountID.SetAttributeNS('schemeID','',TIPO_DOC_CLIENTE);
    ...
    // SELLER
    {cpe.SellerSupplierParty.Party.PartyIdentification.Add;
       cpe.SellerSupplierParty.Party.PartyIdentification.Items[0].ID.Text:=NRO_DOC_CLIENTE;
       cpe.SellerSupplierParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeID','',TIPO_DOC_CLIENTE);
       cpe.SellerSupplierParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeName','','Documento de Identidad');
       cpe.SellerSupplierParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeAgencyName','','PE:SUNAT');
       cpe.SellerSupplierParty.Party.PartyIdentification.Items[0].ID.SetAttributeNS('schemeURI','','urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06');

    cpe.SellerSupplierParty.Party.PartyLegalEntity.Add;
       cpe.SellerSupplierParty.Party.PartyLegalEntity.Items[0].RegistrationName.Text:=EliminaProhibidos(RAZON_NOMBRE_CLIENTE);
    }

    //DATOS DEL ENVIO
    ///DespatchAdvice/cac:Shipment/cac:TransportHandlingUnit/cac:TransportEquipment/cbc:ID


    cpe.Shipment.ID.Text:='SUNAT_Envio';
      cpe.Shipment.HandlingCode.Text:=completar2(DM1.ZGuiaRem.Fields[22].AsString);    //catalogo 20 codigo
         cpe.Shipment.HandlingCode.SetAttributeNS('listAgencyName','','PE:SUNAT');
         cpe.Shipment.HandlingCode.SetAttributeNS('listName','','Motivo de traslado');
         cpe.Shipment.HandlingCode.SetAttributeNS('listURI','','urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo20');
      ...
      cpe.Shipment.ShipmentStage.Add;
         cpe.Shipment.ShipmentStage.Items[0].ID.text:='1';
         cpe.Shipment.ShipmentStage.Items[0].TransportModeCode.Text:=DM1.ZGuiaRem.Fields[26].AsString;  // cat 18 mod trasl 01 publico, 02 privado
            cpe.Shipment.ShipmentStage.Items[0].TransportModeCode.SetAttributeNS('listAgencyName','','PE:SUNAT');
            cpe.Shipment.ShipmentStage.Items[0].TransportModeCode.SetAttributeNS('listName','','Modalidad de traslado');
            cpe.Shipment.ShipmentStage.Items[0].TransportModeCode.SetAttributeNS('listURI','','urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo18');
        ...
        ActivaDM1ConsultaSQL('select apellidopaterno,apellidomaterno,nombre from persona where dni="'+SplitCadena(DM1.ZGuiaRem.Fields[27].AsString,',',1)+'"',false);
        cpe.Shipment.ShipmentStage.Items[0].DriverPerson.add;
            cpe.Shipment.ShipmentStage.Items[0].DriverPerson.items[0].ID.text:=SplitCadena(DM1.ZGuiaRem.Fields[27].AsString,',',1);
                cpe.Shipment.ShipmentStage.Items[0].DriverPerson.items[0].ID.SetAttributeNS('schemeID','','1');
                cpe.Shipment.ShipmentStage.Items[0].DriverPerson.items[0].ID.SetAttributeNS('schemeName','','Documento de Identidad');
                cpe.Shipment.ShipmentStage.Items[0].DriverPerson.items[0].ID.SetAttributeNS('schemeAgencyName','','PE:SUNAT');
                cpe.Shipment.ShipmentStage.Items[0].DriverPerson.items[0].ID.SetAttributeNS('schemeURI','','urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06');

            cpe.Shipment.ShipmentStage.Items[0].DriverPerson.items[0].FirstName.text:=DM1.ConsultaSQL.Fields[2].AsString;
            cpe.Shipment.ShipmentStage.Items[0].DriverPerson.items[0].FamilyName.text:=DM1.ConsultaSQL.Fields[0].AsString;
            cpe.Shipment.ShipmentStage.Items[0].DriverPerson.items[0].JobTitle.text:='Principal';
            cpe.Shipment.ShipmentStage.Items[0].DriverPerson.items[0].IdentityDocumentReference.add;
                cpe.Shipment.ShipmentStage.Items[0].DriverPerson.items[0].IdentityDocumentReference.items[0].ID.text:=DM1.ZGuiaRem.Fields[11].AsString; //brevete
        DM1.ConsultaSQL.Close;

      if DM1.ZGuiaRem.Fields[26].AsString='01' then // transporte publico   o privado
        begin
      ...
      //Delivery Address Direccion de entrega   DESTINO
      cpe.Shipment.Delivery.DeliveryAddress.ID.Text:=DM1.ZGuiaRem.Fields[29].AsString;
          cpe.Shipment.Delivery.DeliveryAddress.ID.SetAttributeNS('schemeName','','Ubigeos');
          cpe.Shipment.Delivery.DeliveryAddress.ID.SetAttributeNS('schemeAgencyName','','PE:INEI');
      cpe.Shipment.Delivery.DeliveryAddress.AddressLine.Add;
          cpe.Shipment.Delivery.DeliveryAddress.AddressLine.Items[0].Line.text:=EliminaProhibidos(DM1.ZGuiaRem.Fields[5].AsString);

      // Despatch Address Direccion de Envio  ORIGEN
      cpe.Shipment.Delivery.DespatcH.DespatchAddress.ID.Text:=DM1.ZGuiaRem.Fields[28].AsString;
          cpe.Shipment.Delivery.DespatcH.DespatchAddress.ID.SetAttributeNS('schemeName','','Ubigeos');
          cpe.Shipment.Delivery.DespatcH.DespatchAddress.ID.SetAttributeNS('schemeAgencyName','','PE:INEI');
      cpe.Shipment.Delivery.DespatcH.DespatchAddress.AddressLine.Add;
          cpe.Shipment.Delivery.DespatcH.DespatchAddress.AddressLine.Items[0].Line.text:=EliminaProhibidos(DM1.ZGuiaRem.Fields[4].AsString);
      ...
      {if (not concdata) then
        ...
        begin
          xmldoc.Active:=True;
          NodoDestino :=XMLDoc.CreateNode (DM1.ZGuiaRem.Fields[5].AsString, ntCDATA);
          cpe.Shipment.Delivery.DeliveryAddress.StreetName.ChildNodes.Add(NodoDestino);
        end;}


      // varias placas o una sola 2023/06
      //Reportes.Hoja.Lines.Add('-----------------------------');
      //Reportes.Hoja.Lines.Add(DM1.ZGuiaRem.SQL.Text);
      if Trim(DM1.ZGuiaRem.Fields[8].AsString)<>'' then
       begin

         //Reportes.Hoja.Lines.Add(DM1.ZGuiaRem.Fields[8].AsString);
         //Reportes.Hoja.Lines.Add('****');

         for i := 1 to 4 do
           begin
             vplaca:=Trim(SplitCadena(DM1.ZGuiaRem.Fields[8].AsString,'/',i));
             Reportes.Hoja.Lines.Add(vplaca);
             if vplaca<>'' then
               begin
                  vplaca:=copy(StrCom( vplaca ,'-',''),1,8);
                  //Reportes.Hoja.Lines.Add(vplaca);
                  if (length(Vplaca)>=6) then
                   begin
                     cpe.Shipment.TransportHandlingUnit.Add;
                     cpe.Shipment.TransportHandlingUnit.Items[i-1].TransportEquipment.add;
                     cpe.Shipment.TransportHandlingUnit.Items[i-1].TransportEquipment.items[0].ID.Text:=vplaca;
                     //showmessage(vplaca)
                   end;
               end;
           end;
      ...
      if (not concdata) then
        ...
        begin
          xmldoc.Active:=True;
          NodoOrigen :=XMLDoc.CreateNode (DM1.ZGuiaRem.Fields[4].AsString, ntCDATA);
          cpe.Shipment.OriginAddress.StreetName.ChildNodes.Add(NodoOrigen);
        end;
      }
     ...
     begin
      ...
      cpe.DespatchLine.Add;
        ...
        cpe.DespatchLine.Items[i].DeliveredQuantity.Text:=StrR2(ValR(DM1.ZDetVen.fields[1].AsString),16,CEROS_XML_CANTIDAD);
           ...
           cpe.DespatchLine.Items[i].DeliveredQuantity.SetAttributeNS('unitCodeListID','','UN/ECE rec 20');
           cpe.DespatchLine.Items[i].DeliveredQuantity.SetAttributeNS('unitCodeListAgencyName','','United Nations Economic Commission for Europe');

        cpe.DespatchLine.Items[i].OrderLineReference.Add;
           cpe.DespatchLine.Items[i].OrderLineReference.Items[0].LineID.Text:=StrI(i+1);

        cpe.DespatchLine.Items[i].Item.Description.Add;
          cpe.DespatchLine.Items[i].Item.Description.Items[0].Text:=eliminaProhibidos(DM1.ZDetVen.Fields[3].AsString);
...
var cpe : IXMLSummaryDocumentsType;
    i,k : Integer;
    MONEDA : string;
 ...
 begin
   //ShowMessage('Limites '+StrI(LimiteI)+' -> '+StrI(LimiteS)+'  i='+StrI(indice));
  {
  //               0            1            2           3            4           5           6              7               8
  'select a.'+IDDUAL+',a.FECHAEMISION,a.HORA,a.TIPOCOMPROBANTE,a.SERIECO,a.NUMEROCO,a.TIPODOCUMENTO,a.NRODOCUMENTO,a.RAZONNOMBRE, '+
  //  9                   10                              11                             12
  ...
  ' a.PLACA,abs(a.PERCEPCION) AS PERCEPCION,a.RCPTIPO,a.RCPSERIE,a.RCPNUMERO,a.CODIGONOTA,a.MOTIVONOTA,a.TIPOPAGO,a.FECHAVENCIMIENTO '+
        31         32        33         34       35           36
    TIPODSCTO, DSCTOUNIT, IGVTASA, ENVIOSUNAT, DSCTOGLOBAL,CARGOGLOBAL
   //       0            1         2           3          4      5         6        7        8
   'SELECT a.ITEM, a.CANTIDAD, Unidad, a.DESCRIPCION, a.PU, a.PU_OR, a.VVUNIT,  a.DSCTO, a.CARGO, '+
   //   9       10     11          12                13          14         15          16           17
   'a.VVITEM, a.ISC, a.IGV, a.TIPOOPERACION, a.TIPOAFECTACION, a.MONTO, a.SUBTOTAL, a.PRODUCTO, a.CodigoSUNAT '+
  ...
  cpe:=NewSummaryDocuments;

  cpe.UBLExtensions.Add;
  cpe.UBLExtensions.UBLExtension[0].AddChild('ext:ExtensionContent');

  cpe.UBLVersionID.Text:='2.0';
  cpe.CustomizationID.Text:='1.1';

  // identificador del resumen diario va con la EMISION
  cpe.ID.Text:='RC-'+
               Copy(DM1.ZRegVen.Fields[1].AsString,7,4)+
               Copy(DM1.ZRegVen.Fields[1].AsString,4,2)+
               Copy(DM1.ZRegVen.Fields[1].AsString,1,2)+'-'+
               Copy(archivo,Length(archivo)-6,3);  // NumeraResumen(DM1.ZRegVen.Fields[3].AsString);

  // Fecha de EMISION  los comprobantes
  cpe.ReferenceDate.Text:=Copy(DM1.ZRegVen.Fields[1].AsString,7,4)+'-'+
                      Copy(DM1.ZRegVen.Fields[1].AsString,4,2)+'-'+
                      Copy(DM1.ZRegVen.Fields[1].AsString,1,2);   // fecha de emision

  //                12345678
  // 20602654002-RC-20181020-00001.xml

  // fecha de GENERACION del resumen
  {tmpstr:=Copy(archivo,Length(archivo)-17,8); NO FUNCIONA con fechas DIFERENTES
  cpe.IssueDate.Text:=Copy(tmpstr,1,4)+'-'+
                      Copy(tmpstr,5,2)+'-'+
                      Copy(tmpstr,7,2);   // fecha de emision}
  cpe.IssueDate.Text:=Copy(DM1.ZRegVen.Fields[1].AsString,7,4)+'-'+
                      Copy(DM1.ZRegVen.Fields[1].AsString,4,2)+'-'+
                      Copy(DM1.ZRegVen.Fields[1].AsString,1,2);   // fecha de emision

  cpe.Note.Add;
  cpe.Note.Items[0].Text:=eliminaProhibidos('RESUMEN DIARIO');

  cpe.Signature.Add;
     cpe.Signature.Items[0].ID.Text:=cpe.ID.Text;
     cpe.Signature.Items[0].SignatoryParty.PartyIdentification.Add;
       cpe.Signature.Items[0].SignatoryParty.PartyIdentification.Items[0].ID.Text:=RUC_EMPRESA;
     cpe.Signature.Items[0].SignatoryParty.PartyName.Add;
       if not concdata then
   ...
   cpe.AccountingSupplierParty.CustomerAssignedAccountID.Text:=RUC_EMPRESA;
   cpe.AccountingSupplierParty.AdditionalAccountID.Add;
     cpe.AccountingSupplierParty.AdditionalAccountID.Items[0].Text:='6';
   cpe.AccountingSupplierParty.Party.PartyLegalEntity.Add;
     if not concdata then
       cpe.AccountingSupplierParty.Party.PartyLegalEntity.Items[0].RegistrationName.Text:=eliminaProhibidos(RAZON_EMPRESA)
     else
       cpe.AccountingSupplierParty.Party.PartyLegalEntity.Items[0].RegistrationName.ChildNodes.add(NodoRazonEmpresa2);

   BtnLevantarDetallesVentaClick(nil);
   //SE DETALLAN A CONTINUACION UNO A UNO LOS COMPROBANTES
   {DM1.ZDetVen.First;
    for i := 0 to DM1.ZDetVen.RecordCount-1 do
     begin
      if i>0 then DM1.ZDetVen.Next;}
   SUMRESUMEN:=0;
   DM1.ZDetVen.First;
   DM1.ZDetVen.MoveBy(LimiteI-1);
   //res_dual[indice]:=ValI(DM1.ZDetVen.Fields[0].ASString);
   res_archivo[indice].dual:=ValI(DM1.ZDetVen.Fields[0].ASString);
   for i := 0 to (LimiteS-LimiteI) do
     begin
      if i>0 then DM1.ZDetVen.Next;
      cpe.SummaryDocumentsLine.Add;
        cpe.SummaryDocumentsLine.Items[i].LineID.Text:=StrI(i+1);
        cpe.SummaryDocumentsLine.Items[i].DocumentTypeCode.Text:=CompletarCadena(DM1.ZDetVen.Fields[3].AsString,'0',2);
        cpe.SummaryDocumentsLine.Items[i].ID.Text:=DM1.ZDetVen.Fields[4].AsString+'-'+CompletarCadena(DM1.ZDetVen.Fields[5].AsString,'0',DIGITOS_NUMCOMPROBANTE);            // serie - numero

        if (DM1.ZDetVen.Fields[7].AsString='123') or (DM1.ZDetVen.Fields[7].AsString='9999')
        or (DM1.ZDetVen.Fields[7].AsString='999') or (DM1.ZDetVen.Fields[7].AsString='00000123')
        or (DM1.ZDetVen.Fields[7].AsString='99999999') or (DM1.ZDetVen.Fields[7].AsString='00000000')  //2024
        then
          cpe.SummaryDocumentsLine.Items[i].AccountingCustomerParty.CustomerAssignedAccountID.Text:='00000000' // no domiciliado  00000000 OSE     - SUNAT
        else
          cpe.SummaryDocumentsLine.Items[i].AccountingCustomerParty.CustomerAssignedAccountID.Text:=DM1.ZDetVen.Fields[7].AsString; // nro doc

        cpe.SummaryDocumentsLine.Items[i].AccountingCustomerParty.AdditionalAccountID.Add;
        if DM1.ZDetVen.Fields[6].AsString='0' then
          cpe.SummaryDocumentsLine.Items[i].AccountingCustomerParty.AdditionalAccountID.Items[0].Text:='-' // tipo doc no domiciliado para OSE una raya, para SUNAT 0
        else
          cpe.SummaryDocumentsLine.Items[i].AccountingCustomerParty.AdditionalAccountID.Items[0].Text:=DM1.ZDetVen.Fields[6].AsString; // tipo doc

        // SI ES NOTA DE CREDITO SE AGREGA LOS RCP
        if (DM1.ZDetVen.Fields[3].AsString='7') OR  (DM1.ZDetVen.Fields[3].AsString='8')  then
          BEGIN
           cpe.SummaryDocumentsLine.Items[i].BillingReference.InvoiceDocumentReference.ID.Text:=DM1.ZDetVen.Fields[25].AsString+'-'+CompletarCadena(DM1.ZDetVen.Fields[26].AsString,'0',DIGITOS_NUMCOMPROBANTE);
           cpe.SummaryDocumentsLine.Items[i].BillingReference.InvoiceDocumentReference.DocumentTypeCode.Text:=CompletarCadena(DM1.ZDetVen.Fields[24].AsString,'0',2);
          END;

        cpe.SummaryDocumentsLine.Items[i].Status.ConditionCode.Text:='1';  // condicion del DOC Cat 19   1 adicionar  2 modificar  3 anular

        //IMPORTE TOTAL
        cpe.SummaryDocumentsLine.Items[i].TotalAmount.Text:=StrR2(ValR(DM1.ZDetVen.fields[19].AsString),12,CEROS_XML);
        cpe.SummaryDocumentsLine.Items[i].TotalAmount.SetAttributeNS('currencyID','',MONEDA);
        SUMRESUMEN:=SUMRESUMEN+VALR(DM1.ZDetVen.fields[19].AsString);

        k:=-1;
        // operaciones gravadas
        if ValR(DM1.ZDetVen.fields[10].AsString)<>0 then
          begin
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Add;  Inc(k);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].PaidAmount.Text:=StrR2(ValR(DM1.ZDetVen.fields[10].AsString),12,CEROS_XML);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].PaidAmount.SetAttributeNS('currencyID','',MONEDA);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].InstructionID.Text:='01';
          end;

        // operaciones inafectas
        if ValR(DM1.ZDetVen.fields[11].AsString)<>0 then
          begin
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Add;  Inc(k);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].PaidAmount.Text:=StrR2(ValR(DM1.ZDetVen.fields[11].AsString),12,CEROS_XML);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].PaidAmount.SetAttributeNS('currencyID','',MONEDA);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].InstructionID.Text:='03';
          end;

        // operaciones exoneradas
        if ValR(DM1.ZDetVen.fields[12].AsString)<>0 then
          begin
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Add;  Inc(k);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].PaidAmount.Text:=StrR2(ValR(DM1.ZDetVen.fields[12].AsString),12,CEROS_XML);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].PaidAmount.SetAttributeNS('currencyID','',MONEDA);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].InstructionID.Text:='02';
          end;

        // operaciones gratuitas
        if ValR(DM1.ZDetVen.fields[13].AsString)<>0 then
          begin
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Add;  Inc(k);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].PaidAmount.Text:=StrR2(ValR(DM1.ZDetVen.fields[13].AsString),12,CEROS_XML);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].PaidAmount.SetAttributeNS('currencyID','',MONEDA);
            cpe.SummaryDocumentsLine.Items[i].BillingPayment.Items[k].InstructionID.Text:='05';
          end;

        // <!-- otros cargos del item -->
        if ValR(DM1.ZDetVen.fields[18].AsString)<>0 then
          begin
            cpe.SummaryDocumentsLine.Items[i].AllowanceCharge.Add;
            cpe.SummaryDocumentsLine.Items[i].AllowanceCharge.Items[0].ChargeIndicator.Text:='true';
            cpe.SummaryDocumentsLine.Items[i].AllowanceCharge.Items[0].Amount.Text:=StrR2(ValR(DM1.ZDetVen.fields[18].AsString),12,CEROS_XML);
            cpe.SummaryDocumentsLine.Items[i].AllowanceCharge.Items[0].Amount.SetAttributeNS('currencyID','',MONEDA);
          end;

        // <!-- Descuentos del item -->


        //<!-- Total IGV --> y otros impuestos
        cpe.SummaryDocumentsLine.Items[i].TaxTotal.Add;                                              //IGV+ISC+otros tributos NO ESTOY SUMANDO EL ICBPER
          cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxAmount.Text:=StrR2(ValR(DM1.ZDetVen.fields[14].AsString),12,CEROS_XML);
          cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxAmount.SetAttributeNS('currencyID','',MONEDA);
          // IGV que normalmente va a en cero, excepto en las Gravadas
          cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Add;
            cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Items[0].TaxAmount.Text:=StrR2(ValR(DM1.ZDetVen.fields[14].AsString),12,CEROS_XML);
            cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Items[0].TaxAmount.SetAttributeNS('currencyID','',MONEDA);
            cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Items[0].TaxCategory.TaxScheme.ID.Text:='1000'; // cat 05
            cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Items[0].TaxCategory.TaxScheme.Name.Text:='IGV'; // cat 05
            cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Items[0].TaxCategory.TaxScheme.TaxTypeCode.Text:='VAT';
          //ICBPER 31/12/2019
          cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Add;
            cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Items[1].TaxAmount.Text:=StrR2(ValR(DM1.ZDetVen.fields[16].AsString),12,CEROS_XML);
            cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Items[1].TaxAmount.SetAttributeNS('currencyID','',MONEDA);
            cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Items[1].TaxCategory.TaxScheme.ID.Text:='7152';
            cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Items[1].TaxCategory.TaxScheme.Name.Text:='ICBPER';
            cpe.SummaryDocumentsLine.Items[i].TaxTotal.Items[0].TaxSubtotal.Items[1].TaxCategory.TaxScheme.TaxTypeCode.Text:='OTH';
     end;
    res_archivo[indice].dual2:=ValI(DM1.ZDetVen.Fields[0].ASString);
    //25/10/2019 dual 2 al final del detventas
...
begin
  ...
  defSvc :=SV_BETA_SRV;
  defPrt :=SV_BETA_PORT;
      //15/07 Maestro Envio de Guias
      if ((Copy(archivo,13,2)='09') OR (Copy(archivo,13,2)='31')) and (SV_BETA_GUIAS_WSDL<>'') then
       begin
         defWSDL:=SV_BETA_GUIAS_WSDL;
         defURL :=SV_BETA_GUIAS_URL; // quitamos el ?
       end;
  // recarga de datos
  ...
  if Not FileExists(completardir(pathenvio,Archivo)) then
   begin
      ShowMessage('No_se encuentra la ruta '+pathenvio);
      ShowMessage('No_se encuentra el archivo '+Archivo);
   end
  else
   begin
      envio:= FileToByteArray(pathenvio+Archivo);
      // envio
      error:=False;
      try
        //1234567890123
        //10405242112-01
        if (ticket='') or (ticket='0') then
         begin
          if Copy(archivo,13,1)='R' then // resumen
            begin
              //ShowMessage('Send Summary beta');
              SEND_RESPOSE:=tBillServ.sendSummary(Archivo,envio,'');
         ...
         begin
           //ShowMessage('Get Status Beta '+ticket);
        ...
        on E : Exception do
         begin
           ...
           SEND_ERROR:=Copy(E.Message,1,255);
           error:=True;
         end;
      end;
      ...
      if not Error then
       begin
        ...
        if SEND_RESPOSE='' then
          ByteArrayToFIle(respuesta, pathCDR+'R'+archivo );
       end;
